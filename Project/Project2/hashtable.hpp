...
protected:                                                                  // DO NOT USE private HERE!
    ...
    size_t findMinimumBucketSize(size_t bucketSize) const {
        ...
        // greater than floor(tableSize / maxLoadFactor)
        size_t lowerBound = (size_t)floor((double)this->size()/this->getMaxLoadFactor() + 1);
        // not less than (i.e. greater or equal to) the parameter bucketSize
        if (bucketSize > lowerBound)
        {
            lowerBound = bucketSize;
        }
        // a (prime) number defined in HashPrime (hash_prime.hpp)
        auto itMin = std::lower_bound(HashPrime::g_a_sizes, HashPrime::g_a_sizes + HashPrime::num_distinct_sizes - 1, lowerBound);
        // if no such bucket size can be found
        if (lowerBound > HashPrime::g_a_sizes[HashPrime::num_distinct_sizes - 1])
        {
...
public:
    ...
    HashTable &operator=(const HashTable &that) {
        // TODO: implement this function
        // TASK: deep copy every element of that into this
    ...
    Iterator find(const Key &key) {
        ...
        auto i = it.bucketIt->begin();
        while (i != it.bucketIt->end())
        {
    ...
    bool insert(const Iterator &it, const Key &key, const Value &value) {
        ...
        if (it.endFlag)
        {
            ...
            this->tableSize++;
            this->firstBucketIt = min(it.bucketIt, this->firstBucketIt);
    ...
    Iterator erase(const Iterator &it) {
        ...
        it.bucketIt->erase_after(it.listItBefore);
        this->tableSize--;
        if (this->firstBucketIt == it.bucketIt)
        {
            this->firstBucketIt = this->buckets.end();
            auto i = buckets.begin();
            while (i != buckets.end())
            {
                if (!i->empty())
                {
                    this->firstBucketIt = i;
                    return it;
                }
                i++;
            }
    ...
    void rehash(size_t bucketSize) {
        ...
        // TODO: implement this function
        HashTableData tmp(this->buckets);
        this->buckets.clear();
